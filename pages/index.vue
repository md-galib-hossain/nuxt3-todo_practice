      ...
      <ul v-if="todos.length" class="space-y-2">
        ...
        >
          ...
          <div class="flex space-x-2">
            <button
              @click="markAsComplete(item._id, index)"
              :class="[
                item.completed
            ...
            <button
              @click="removeFromTodo(item._id,index)"
...
const newTodo = ref<{ title: string; description: string }>({
  title: "",
  description: "",
});
...
async function addTodo() {
  ...
  try {
    ...
    if (error.data?.errorSources) {
      errorMessages.value = error.data.errorSources.map(
        (item: { path: string; message: string }) => item.message
      );
    } else {
...
async function markAsComplete(id: string, index: number) {
  errorMessages.value = [];
  console.log(todos)
  try {
    const data: { data: Todo } = await $fetch(`${config}/todos/${id}`, {
      method: "PATCH",
      body: {
...
async function removeFromTodo(id:string,index: number) {
  errorMessages.value = []; // Clear any existing error messages

try {
  await $fetch(`${config}/todos/soft-delete/${id}`, {
    method: "PATCH", 
  });
